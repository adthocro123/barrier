...
void ZeroconfService::serverDetected(const QList<ZeroconfRecord>& list)
{
    ...
    for (const ZeroconfRecord& record : list) {
        ...
        m_pMainWindow->appendLogInfo(tr("zeroconf server detected: %1").arg(
            record.serviceName));
        serverNames.append(record.serviceName);
    }

    serverNames.removeDuplicates();
    serverNames.sort(Qt::CaseInsensitive);
    m_pMainWindow->serversDetected(serverNames);
}
...
{
    m_pMainWindow->discoveryFailed(errorCode);
}
