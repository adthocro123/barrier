...
QString connection_help_for_log_line(const QString& line)
{
    const auto normalized = line.toLower();

    if (normalized.contains("server refused client with name") ||
        normalized.contains("unrecognised client name")) {
        return QCoreApplication::translate(
            "MainWindow",
            "This computer's screen name is not configured on the server. "
            "Open Configure Server and add it exactly as shown above.");
    }

    if (normalized.contains("ssl error") ||
        normalized.contains("eof violates ssl protocol") ||
        normalized.contains("failed to verify server certificate")) {
        return QCoreApplication::translate(
            "MainWindow",
            "Secure connection failed. Confirm both computers use the same "
            "security setting, then reconnect and approve the fingerprint.");
    }

    if (normalized.contains("disconnected from server")) {
        return QCoreApplication::translate(
            "MainWindow",
            "The server closed the connection. Confirm both computers run the "
            "same app version and use matching security settings.");
    }
    ...
    if (normalized.contains("connection refused") ||
        ...
        return QCoreApplication::translate(
            "MainWindow",
            "Can't reach the server. Check its address, firewall, and that the "
            "server is running.");
    }
    ...
    m_pLogWindow(new LogWindow(nullptr))
{
    ...
    ui_->m_pComboServerList->setVisible(appConfig.autoConfig());
    ui_->m_pComboServerList->setEnabled(false);
    ui_->m_pButtonRefreshDiscovery->setVisible(appConfig.autoConfig());
    ui_->m_pLabelDiscoveryStatus->setVisible(appConfig.autoConfig());
    if (appConfig.autoConfig()) {
        ui_->m_pLabelDiscoveryStatus->setText(
            tr("Searching your local network for a server…"));
    }
...
void MainWindow::updateFromLogLine(const QString &line)
{
    ...
    const auto help = connection_help_for_log_line(line);
    if (!help.isEmpty()) {
        setStatus(help);
    }
}
...
bool MainWindow::clientArgs(QStringList& args, QString& app)
{
    ...
    if (ui_->m_pCheckBoxAutoConfig->isChecked()) {
        ...
        if (!m_SuppressEmptyServerWarning) {
            QMessageBox::information(
                this, tr("Still looking for a server"),
                tr("No nearby server has been found yet. Make sure the server "
                   "computer is running Barrier Next on the same local network, "
                   "then select Scan again. You can also turn off automatic "
                   "discovery and enter its address manually."));
        }
        return false;
    }

    if (ui_->m_pLineEditHostname->text().isEmpty()) {
        show();
        if (!m_SuppressEmptyServerWarning) {
    ...
    if (isBonjourRunning()) {
        if (!m_AppConfig->wizardShouldRun()) {
            ...
            if (m_AppConfig->autoConfig() || app_role() == AppRole::Server) {
                ...
                if (app_role() == AppRole::Client && m_AppConfig->autoConfig()) {
                    ui_->m_pLabelDiscoveryStatus->setText(
                        tr("Searching your local network for a server…"));

                    QTimer::singleShot(3500, this, [this]() {
                        if (app_role() == AppRole::Client &&
                            ui_->m_pCheckBoxAutoConfig->isChecked() &&
                            ui_->m_pComboServerList->count() == 0) {
                            ui_->m_pLabelDiscoveryStatus->setText(
                                tr("No nearby server found yet. Check that both "
                                   "computers are on the same network. Turn off "
                                   "Find server automatically to enter an address."));
                        }
                    });
                }
            }
...
void MainWindow::serversDetected(const QStringList& names)
{
    const QString previousSelection = ui_->m_pComboServerList->currentText();
    QString nextSelection = previousSelection;

    if (!names.contains(nextSelection)) {
        nextSelection = names.isEmpty() ? QString() : names.first();
    }

    {
        const QSignalBlocker blocker(ui_->m_pComboServerList);
        ui_->m_pComboServerList->clear();
        ui_->m_pComboServerList->addItems(names);

        if (!nextSelection.isEmpty()) {
            ui_->m_pComboServerList->setCurrentText(nextSelection);
        }
    }

    ui_->m_pComboServerList->setEnabled(!names.isEmpty());

    if (names.isEmpty()) {
        ui_->m_pLabelDiscoveryStatus->setText(
            tr("No nearby server found yet. Check that both computers are on "
               "the same network. Turn off Find server automatically to enter "
               "an address."));
    }
    else if (names.size() == 1) {
        ui_->m_pLabelDiscoveryStatus->setText(
            tr("Found %1. It will be used automatically.").arg(names.first()));
    }
    else {
        ui_->m_pLabelDiscoveryStatus->setText(
            tr("Found %1 nearby servers. Choose the one you want to control this computer.")
                .arg(names.size()));
    }

    if (!nextSelection.isEmpty() && nextSelection != previousSelection) {
        // Preserve the existing auto-connect behaviour, but only restart when
        // the selected server really changes.
        comboServerList_currentIndexChanged(nextSelection);
    }
}

void MainWindow::discoveryFailed(int errorCode)
{
    appendLogError(tr("Local network discovery failed with error code %1.")
                       .arg(errorCode));
    ui_->m_pLabelDiscoveryStatus->setText(
        tr("Automatic discovery is unavailable. Enter the server address "
           "manually, or select Scan again."));
}
...
void MainWindow::on_m_pButtonRefreshDiscovery_clicked()
{
    ui_->m_pLabelDiscoveryStatus->setText(
        tr("Searching your local network for a server…"));
    updateZeroconfService();
...
void MainWindow::on_m_pCheckBoxAutoConfig_toggled(bool checked)
{
    ...
    ui_->m_pComboServerList->setVisible(checked);
    ui_->m_pButtonRefreshDiscovery->setVisible(checked);
    ui_->m_pLabelDiscoveryStatus->setVisible(checked);
