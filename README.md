# Barrier Next (working title)

Barrier Next is a community continuation of the Barrier/InputLeap codebase,
focused first on dependable Windows-to-macOS sharing and a setup experience that
does not require reading logs. It is currently pre-release software.

The project preserves the full upstream history and GPLv2 licensing. See the
[roadmap](docs/ROADMAP.md) for the first release plan.

## Current development goals

- Friendly first-run setup and actionable connection errors
- A universal macOS installer and a current Windows x64 installer
- Secure connections that work without manual certificate repair
- A small, understandable project for new contributors

## Upstream history

This work is based on InputLeap, which was maintained by Barrier's active
maintainers after Barrier became unmaintained. Credit belongs to the Barrier,
InputLeap, Synergy, and Deskflow contributors whose work made this continuation
possible.

---

## Upstream InputLeap documentation
...
1. Install and run Barrier Next on each machine that will be sharing.
...
4. Ensure the "screen name" matches exactly (case-sensitive) for each configured
    screen -- the clients' Barrier Next windows will tell you their screen names
    (just above the server IP).
5. On each client, enable **Find server automatically**. Barrier Next uses
    mDNS/Bonjour to show nearby servers, selects the only result automatically,
    and keeps the list current as computers join or leave the network. It does
    not probe every IP address. If discovery is blocked by the network, turn it
    off and enter the server address manually.
